...
export const SwipeableContainer = ({ celebrities, onVote, disabled }: SwipeableContainerProps) => {
      ...
      <div className="grid grid-cols-2 gap-3">
        {celebrities.map((celebrity, index) => (
          <div key={celebrity.id} className="relative">
            ...
            <Card
              className="overflow-hidden border border-border/30 rounded-2xl cursor-pointer"
              onClick={() => {
                if (!disabled) {
            ...
            >
              ...
              <div className="p-3 bg-card">
                <h3 className="text-sm font-serif font-semibold mb-1 truncate">{celebrity.name}</h3>
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>{Math.round(celebrity.elo_rating)}</span>
                  <span>{celebrity.games_played}</span>
                </div>
              </div>
            </Card>
          </div>
